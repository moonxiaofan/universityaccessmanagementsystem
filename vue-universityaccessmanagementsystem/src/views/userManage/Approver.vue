...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      this.$http
      .post('/user/getAllApproverList', param)
      .then((res) => {
    ...
    handleEdit(index, row) {
      this.form = row
      this.dialogTitle = '修改审批人员'
      this.dialogFormVisible = true
    },
    handleDelete(index, row) {
      this.form = row
      this.$confirm('此操作将删除该审批人员, 是否继续?', '提示', {
      ...
      this.$http
        .post('/user/editApproverUser', param)
        .then((res) => {
    ...
    addButton() {
      this.dialogFormVisible = true;
      this.dialogTitle='新增审批人员'
    },
    editAcademy() {
      this.$http
        .post('/user/editApproverUser', this.form)
        .then((res) => {
          if(res.data.result === "success") {
            this.$message({
