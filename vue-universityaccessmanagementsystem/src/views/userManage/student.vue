...
export default {
  ...
  methods: {
    ...
    search() {
      ...
      this.$http
      .post('/user/getAllStudentList', param)
      .then((res) => {
    ...
    handleEdit(index, row) {
      this.form = row
      this.dialogTitle = '修改学生人员'
      this.dialogFormVisible = true
    },
    handleDelete(index, row) {
      this.form = row
      this.$confirm('此操作将删除该学生人员, 是否继续?', '提示', {
      ...
      this.$http
        .post('/user/editStudentUser', param)
        .then((res) => {
    ...
    addButton() {
      this.dialogFormVisible = true;
      this.dialogTitle='新增学生人员'
    },
    editAcademy() {
      this.$http
        .post('/user/editStudentUser', this.form)
        .then((res) => {
          if(res.data.result === "success") {
            this.$message({
